    ...
    <header
      ...
      <div className="max-w-screen mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex justify-between items-center h-20">
          {/* Logo (clickable) */}
          <div onClick={() => navigate("/")} className="flex items-center space-x-1 cursor-pointer">
            
            <img src={Logo} alt="CodePaila Logo" className="h-25 w-auto transition-transform duration-300 hover:scale-105 mt-4" />
          ...
          <nav className="hidden md:flex space-x-1">
            ...
            {navItems.map((item) => (
              ...
              <Link key={item.name} to={item.path} className="relative px-4 py-2 text-gray-700 hover:text-blue-600 font-medium transition-all duration-300 group">
                {item.name}
          ...
          {/* Mobile Menu Button */}
          <button className="md:hidden relative z-10 transition-all duration-200" onClick={() => setIsMenuOpen(!isMenuOpen)}>
